import { getFeedbackRequestByToken } from '@/modules/feedback/actions'
import { FeedbackForm } from '@/modules/feedback/components/feedback-form'
import { notFound } from 'next/navigation'
import { CheckCircle2, AlertCircle } from 'lucide-react'

export default async function PublicFeedbackPage(props: { params: Promise<{ token: string }> }) {
  ...
  const request = await getFeedbackRequestByToken(token)
      ...
      <div className="min-h-screen flex flex-col items-center justify-center bg-zinc-50 dark:bg-zinc-950 p-4">
        <div className="max-w-md w-full text-center p-10 bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-xl shadow-sm space-y-4">
          ...
          <div className="space-y-2">
            ...
            <p className="text-sm text-zinc-500 dark:text-zinc-400">This feedback link is invalid or does not exist.</p>
          </div>
        </div>
      </div>
    )
      ...
      <div className="min-h-screen flex flex-col items-center justify-center bg-zinc-50 dark:bg-zinc-950 p-4">
        <div className="max-w-md w-full text-center p-10 bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-xl shadow-sm space-y-4">
          ...
          <div className="space-y-2">
            ...
            <p className="text-sm text-zinc-500 dark:text-zinc-400">Feedback for this request has already been recorded. Thank you for your time!</p>
          </div>
        </div>
      </div>
    )
